...
### 新增
- **设置与 README 增加赞助入口**：支持 PayPal、Buy me a coffee、Wise、微信赞赏码、支付宝，设置页点击赞助方式后展示对应二维码和链接

### 修复
- **跟随系统语言识别错误**：改用系统首选语言列表识别中文 / 英文 / 日文 / 韩文，避免中文系统仍显示英文
- **安装后 App 名称缺少空格**：主屏显示名改为 `Photo Cleaner`
- **PayPal 二维码更新**：替换为新的 PayPal 收款二维码，并将 PayPal 放到赞助列表第一位

### 变更
- **首页时间线默认显示最近一年**：时间线底部新增「显示全部时间线」按钮，展开后显示右下角返回顶部按钮
- **README 赞助顺序同步**：中英日韩 README 赞助列表统一 PayPal 优先
...
### 修复
- **智能建议统计不准**：实况照片、视频、自拍改用系统智能相册计数，避免首页显示 0 但进入后有照片
- **智能建议文案与内容不一致**：「陈年截图」改为 90 天以前截图，「社交媒体」改为更准确的「低清图片」
- **潜在可释放百分比一直为 0**：改为按建议清理数量占全库比例计算

### 变更
- **时间线支持查看全部月份**：保留滚动追加更早月份，并在标题右侧新增「显示全部」按钮
...
### 修复
- **大量照片库启动白屏闪退**：照片统计改为后台轻量扫描，减少主线程枚举和大数组驻留
- **缩略图内存占用过高**：限制图片缓存数量和总成本，列表缩略图避免触发 iCloud 原图下载
- **浅色主题图片详情分享按钮看不清**：分享按钮文字改用主文本色

### 变更
- README 截图更新为首页、相册、设置三张当前界面图
...
### 变更
- **「有 N 张待删除」弹窗按钮全部无填充背景**：之前 `highlightedCancel` 用蓝/白填充，改为纯文字
  - 「点错了」浅色主题：**黑字 semibold**
  - 「点错了」深色主题：**白字 semibold**
  - 其它按钮（查看待删除 / 放弃并退出）保持蓝/红文字
- **弹窗深色背景更透明**：去掉 `ultraThinMaterial` 上的半透明深色 overlay，让液态玻璃质感真正凸显
- **首页「智能建议」改横向滚动 + 增加到 6 个**：陈年截图 / 占空间大户 / 视频清理 / 实况照片 / 自拍清理 / 社交媒体
  - 每张卡固定 170pt 宽，可左右滑查看更多
  - 每个建议有不同渐变色（蓝/红/绿/青/粉/紫）

### 新增
- L10n 新增「视频清理」「自拍清理」翻译（陈年截图/占空间大户/实况照片/社交媒体已有）
- `suggestionCard(for:label:symbol:gradient:)` 工厂方法，减少重复代码
